...
|-------------------|----------------|
| Scene 1 hook + scroll | Finethir bleats in `[in sheep]` only; `scroll_speak_animals_modified` in his inventory; `[OFFER_ITEM]` + player **use** from inventory grants beast comprehension; `game.yml` `conversation_item_offers` blocks repeat offers |
