...
class ShockingGraspSpell(Spell):
    ...
    def resolve(self, entity, battle, spell_action):
        ...
        if hit:
            ...
            damage_roll = DieRoll.roll(f"{level}d8", crit=attack_roll.nat_20(), battle=battle, entity=entity, description=t('dice_roll.spells.generic_damage', spell=t('spell.shocking_grasp')))
            return [
                {
                    'source': entity,
                    'target': target,
