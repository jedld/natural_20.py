class MapRenderer:
    ...
    def render(self, entity=None, line_of_sight=None, path=[], acrobatics_checks=[], athletics_checks=[], select_pos=None,
               update_on_drop=True, range_value=None, range_cutoff=False, path_char=None, highlight={}, viewport_size=None, top_position=[0, 0]):
    ...
    def token(self, entity, pos_x, pos_y):
        if entity['entity'].token is not None:
            m_x, m_y = self.map.entities[entity['entity']]

            if pos_x - m_x < 0 or pos_y - m_y < 0:
                return None
            if pos_x - m_x >= len(entity['entity'].token()) or pos_y - m_y >= len(entity['entity'].token()[0]):
                return None
    ...
    def any_line_of_sight(self, line_of_sight, entity):
        ...
        if isinstance(line_of_sight, list):
            for l in line_of_sight:
                if self.battle.can_see(l, entity):
                    return True
        else:
            if self.battle.can_see(line_of_sight, entity):
                return True
