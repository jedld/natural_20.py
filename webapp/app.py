...
def conversation_visible_whisper_usernames(source_entity, audible_usernames=None):
    ...
    candidate_usernames.update({str(username).lower() for username in current_game.username_to_sid.keys() if username})
    ...
    for username in candidate_usernames:
        ...
        listener = conversation_listener_for_username(username)
        if listener is None or listener == source_entity:
            continue

        try:
            listener_map = current_game.get_map_for_entity(listener)
        except Exception:
            listener_map = None
...
def conversation_payload(entity, message, targets=None, volume=None, distance_ft=None, mentioned_targets=None, language=None, visual_only_usernames=None):
...
def conversation_listener_for_username(username):
    try:
        listener = current_game.get_pov_entity_for_user(username)
    except Exception:
        listener = None
    ...
    try:
        controlled_entities = entities_controlled_by(username)
    except Exception:
        controlled_entities = []
    return controlled_entities[0] if controlled_entities else None


def listener_understands_language(listener, language):
    if listener is None:
        return False
    try:
...
def render_conversation_payload_for_username(username, source_entity, payload):
    ...
    if username in set(entity_owners(source_entity)):
        return payload_for_user
    ...
    listener = conversation_listener_for_username(username)
    if listener_understands_language(listener, language):
        return payload_for_user
...
def resolve_conversation_targets(source_entity, primary_targets=None, message=None):
    explicit_targets = []
    for entity_uid in primary_targets or []:
        target = current_game.get_entity_by_uid(entity_uid)
        if target is None:
            try:
                target = game_session.entity_by_uid(entity_uid)
            except Exception:
                target = None
...
def select_conversation_responders(processed_conversations, source_entity, targeted_entities=None, latest_message=None, language='common', volume='normal', llm_conversation_handler=None):
...
def emit_conversation_to_usernames(usernames, payload, source_entity=None):
    for recipient_username in usernames:
        rendered_payload = render_conversation_payload_for_username(recipient_username, source_entity, payload) if source_entity is not None else payload
        sids = current_game.username_to_sid.get(recipient_username, [])
        for sid in sids:
            socketio.emit('message', {'type': 'conversation', 'message': rendered_payload}, to=sid)


def conversation_status_summary(entity):
    statuses = []
    try:
...
def conversation_effect_summary(entity):
    effect_names = []
...
def conversation_goal_summary(entity):
    try:
        get_goal = getattr(current_game, 'get_short_term_goal', None)
...
def conversation_attitude_toward_speaker(receiver, speaker):
    ...
    if receiver_group and speaker_group:
        if receiver_group == speaker_group:
            return 'allied'
        try:
            if game_session.opposing(receiver_group, speaker_group):
                return 'hostile'
        except Exception:
            pass
        return 'wary'
    ...
    try:
        receiver_owners = set(entity_owners(receiver))
        speaker_owners = set(entity_owners(speaker))
...
def conversation_pressure_summary(receiver):
    pressures = []
    ...
    statuses = conversation_status_summary(receiver)
    if statuses:
        pressures.append(f"statuses: {', '.join(statuses[:4])}")

    effects = conversation_effect_summary(receiver)
    if effects:
        pressures.append(f"active effects: {', '.join(effects[:3])}")

    goal_text = conversation_goal_summary(receiver)
    if goal_text:
...
def conversation_response_prompt(receiver, speaker):
    target_entities = entity_rag_handler.get_conversation_targets(receiver, speaker=speaker)
    target_handles = []
    ...
    stance_text = conversation_attitude_toward_speaker(receiver, speaker)
    pressure_text = conversation_pressure_summary(receiver)
    return (
...
def effective_talk_volume(message, requested_volume=None, requested_distance_ft=None):
...
def talk():
    ...
    volume = effective_talk_volume(message, requested_volume=data.get('volume'), requested_distance_ft=data.get('distance_ft'))
    ...
    entity_targets, mentioned_targets = resolve_conversation_targets(entity, primary_targets=primary_targets, message=message)
    ...
    speaker_payload = conversation_payload(
        entity,
        message,
    ...
    speaker_audience = conversation_audience_usernames(entity, processed_conversations, targets=delivered_targets)
    visual_whisper_usernames = set()
    if speaker_payload['volume'] == 'whisper':
        visual_whisper_usernames = conversation_visible_whisper_usernames(entity, audible_usernames=speaker_audience)
        if visual_whisper_usernames:
            speaker_payload['visual_only_usernames'] = sorted(visual_whisper_usernames)
    emit_conversation_to_usernames(speaker_audience | visual_whisper_usernames, speaker_payload, source_entity=entity)

    eligible_receivers = select_conversation_responders(
    ...
    for receiver in eligible_receivers:
        ...
        system_prompt += conversation_response_prompt(receiver, entity)
        ...
        reply_plan = entity_rag_handler.build_conversation_response_plan(
            response,
            receiver,
            entity,
        ...
        if reply_plan['skip']:
            entity_rag_handler.apply_response_plan_directives(reply_plan, receiver, speaker=entity)
            continue
        ...
        recipient_usernames = conversation_audience_usernames(receiver, response_conversations, targets=reply_plan['targets'])
        visual_whisper_usernames = set()
        if reply_plan['volume'] == 'whisper':
            visual_whisper_usernames = conversation_visible_whisper_usernames(receiver, audible_usernames=recipient_usernames)
        emit_conversation_to_usernames(
...
@app.route('/conversation_presence')
def conversation_presence():
    entity_id = request.args.get('entity_id')
    if not entity_id:
        username = session.get('username')
        if username:
            current_pov = current_game.get_pov_entity_for_user(username)
    ...
    volume = normalize_speech_mode(mode=request.args.get('volume'), distance_ft=request.args.get('range'))
    distance_ft = speech_distance_for(mode=volume, distance_ft=request.args.get('range'))
    audible = entity_rag_handler.get_nearby_entities(entity, distance_ft, volume=volume, include_extended=True)
...
def nearby_entities():
    entity_id = request.args.get('entity_id')
    volume = normalize_speech_mode(mode=request.args.get('volume'), distance_ft=request.args.get('range'))
    range_ft = speech_distance_for(mode=volume, distance_ft=request.args.get('range'))
    ...
    response = entity_rag_handler.get_nearby_entities(entity, range_ft, volume=volume)
